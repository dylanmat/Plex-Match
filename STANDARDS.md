...
- Permit web-initiated reauthorization only from loopback clients when `PLEX_TOKEN` is an expired JWT, and never return Plex tokens, signed device JWTs, device private keys, or `.env` contents in web responses.
